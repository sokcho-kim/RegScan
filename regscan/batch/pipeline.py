    ...
    try:
        ...
        if settings.ENABLE_LEGACY_WORKERS:
            ...
            try:
                ...
                # MFDS 워커 — 최근 변경분 증분 수집 (DB 적재용)
                # NOTE: daily_scan의 mfds_new는 이 워커가 아니라 DailyScanner가
                # permits_full_*.json을 읽어 생성한다. 그 캐시 갱신은
                # scripts/refresh_mfds_dump.py(또는 run_daily_scan --refresh-mfds)가 담당.
                try:
