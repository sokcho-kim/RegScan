...
### 기사 방향 유형
- **"regulation"**: 규제/정책 변화 — "뭐가 바뀌는데?"가 핵심.
  - 국내: 고시 개정, 급여 기준 변경, 약사법/건보법 개정안, 식약처 안전성 조치
  - 해외: FDA/EMA/PMDA 규제 프레임워크 변경, HTA 정책 변화, 글로벌 법령 동향
  - 반드시 포함: 변경 전후 비교, 시행 시점, 영향 받는 업무/부서
  - 시그널 소스: ASSEMBLY_BILL, MOHW_HEALTH_INSURANCE, MFDS_SAFETY_LETTER, MFDS_PRESS, KHIDI_GLOBAL_INFO(법령·고시/허가·특허)
...
[
  {
    ...
    "structural_info": ["기사 구조에 반드시 포함할 핵심 정보 (삭제 금지)", "예: 기존 대비 변화, 영향 범위, 핵심 수치"],
...
3. **맥락/비교** (1단락, 필수): 기존 대비 뭐가 달라졌는지. 반드시 "A vs B" 형태의 비교를 수치와 함께 포함.
   ...
   - 비교 대상이 없으면 시간축("지난해 같은 기간 대비")이나 비율("전체의 약 X%") 활용.
   - **이 단락이 없으면 기사 불합격.**
...
- **용어 풀이/정의 반복 금지**: 독자는 전문가(의사, 약제팀, 제약 업계)다. "~라는 취지로 읽힌다" 같은 뻔한 풀이는 불합격. 임팩트를 써라.
- **무관 정보 생략**: 편집장 direction과 관계없는 보도자료 항목(공모전, 이벤트 등)은 아예 언급하지 마라.
- **구체적 주어**: "~축에서", "~측면에서", "~관점에서 볼 때" 같은 추상 표현 대신, 구체적 대상(기관명, 약물명, 부서명)을 주어로 써라.
...
## 참조 기사 2: 규제 변경 (regulation 유형은 이 톤을 따르세요)

> **제목:** 급여 상한금액표 개정…항암제 3종 급여 확대, 약제팀 일정 재점검 필요
>
> **리드:** 보건복지부가 5월 1일자로 약제 급여 목록 및 급여 상한금액표를 일부 개정 고시했다. 항암제 3종의 급여 적응증이 확대되면서 약제팀의 급여 청구 기준 재점검이 필요한 상황이다.
>
> **변경 전후:** 기존에 2차 치료 이후에만 급여되던 A약제가 1차 치료까지 확대됐다. 이는 NICE TA 권고(2025년 12월)와 동일한 방향으로, 글로벌 급여 기준과의 정합성을 높인 조치다.
>
> **영향 범위:** 해당 적응증 환자를 진료하는 종합병원 약제팀은 DUR 점검 기준 업데이트와 처방 프로토콜 반영이 필요하다.
>
> **시행 일정:** 5월 1일 시행, 기존 처방 건은 소급 적용 없음. 6월 심평원 점검 예정.

위 기사의 특징: (1) "기존 2차→1차 확대" 변경 전후 비교, (2) "DUR 점검" 구체적 영향 대상, (3) "5월 1일" 시행일 명시, (4) NICE와 교차 비교.
...
async def agent_reporter(
    ...
    for src in sources_used:
        if src in signals:
            relevant_data += format_for_prompt(src, signals[src], include_counts=False) + "\n\n"
    if not relevant_data:
        for src_type, sigs in list(signals.items())[:3]:
            relevant_data += format_for_prompt(src_type, sigs, include_counts=False) + "\n\n"

    grade = story.get("grade", "A")
    article_type = story.get("article_type", "drug")

    # 등급별 작성 지시
    if article_type == "regulation" and grade == "A":
        grade_instruction = """## A등급: 규제 변경 분석 — 반드시 5블록 구조 (700~1,000자)

**블록 1. 변경 사항** (2~3문장): 무엇이, 언제부터, 어떻게 바뀌는가. 고시/법안 번호, 발표 기관, 시행일 명시.
**블록 2. 변경 전후 비교** (1단락): 이전 규정 vs 신규 규정. 반드시 "기존에는 A였으나 이제는 B" 형태. 수치 비교 필수.
**블록 3. 영향 범위** (1단락): 어떤 약물/기업/병원 업무에 영향. "관련 업계" 금지 — "종합병원 약제팀", "바이오시밀러 제조사", "보험심사 담당자" 수준으로 구체적.
**블록 4. 국내 시사점** (1단락): 해외 규제면 국내 제도에의 영향 경로. 국내 규제면 실무 영향. 유사 해외 선례가 있으면 비교.
**블록 5. 시행 일정 및 대응** (1~2문장): 시행일/유예기간, 후속 절차(의견제출, 심의 일정, 하위법령 제정 일정).
...
다음 확인 포인트:"""
    ...
    elif article_type == "regulation":  # B등급 regulation
        grade_instruction = """## B등급: 규제 변경 단신 — 400~600자

핵심 변경 사항 + 변경 전후 비교 + 영향 범위 + 시행 시점 + 대응 포인트.
기사체로 작성하되 간결하게. 반드시 "기존 → 변경" 비교와 시행일 포함."""
...
기사체로 작성하되 간결하게. 배경/비교 블록은 생략 가능."""
    ...
    if article_type == "regulation":
        type_instruction = """
## 규제 기사 특별 규칙
- 변경 전후를 반드시 비교하라. "~로 바뀐다"만 쓰면 불합격. "기존에는 A였으나 이제는 B"가 필수.
- 시행 시점을 명시하라. "향후 시행 예정"은 불합격. 구체적 날짜 또는 "공포 후 6개월"처럼 기간을 쓸 것.
- 영향 받는 대상을 구체적으로 쓰라. "관련 업계"가 아니라 "종합병원 약제팀", "바이오시밀러 제조사", "보험심사 담당자" 수준.
- 해외 규제 변경이면, 국내에 같은/유사 제도가 있는지 반드시 언급하라."""
...
async def generate_articles(
    ...
    # 규제 시그널 우선 슬롯: regulation 유형 기사가 하나도 없으면 강제 추가
    has_regulation = any(
        s.get("article_type") == "regulation" for s in stories
    )
    if not has_regulation:
        _reg_added = False

        # 1순위: ASSEMBLY_BILL (국회 법안)
        _assembly_aliases = {"ASSEMBLY_BILL", "KOREA_LEGISLATION", "ASSEMBLY", "LEGISLATION"}
        if not _reg_added and "ASSEMBLY_BILL" in filtered:
            ...
            if top_bills:
                ...
                stories.append({
                    ...
                    "grade": "B",
                    "grade_reason": "규제 고정 슬롯 — 편집장 미선택 보완 (법안)",
                ...
                _reg_added = True
                logger.info("[규제 고정] 법안 스토리 강제 추가")

        # 2순위: KHIDI_GLOBAL_INFO 중 법령·고시 시그널
        if not _reg_added and "KHIDI_GLOBAL_INFO" in filtered:
            reg_sigs = [
                s for s in filtered["KHIDI_GLOBAL_INFO"]
                if s.get("board") in ("법령 및 고시", "허가 및 특허")
            ][:3]
            if reg_sigs:
                reg_titles = ", ".join(s.get("title", "")[:30] for s in reg_sigs)
                stories.append({
                    "story_id": len(stories) + 1,
                    "article_type": "regulation",
                    "grade": "B",
                    "grade_reason": "규제 고정 슬롯 — 편집장 미선택 보완 (KHIDI 법령·허가)",
                    "read_reason": "최근 글로벌 규제 변경 동향",
                    "direction": "최근 법령·고시·허가 변경의 핵심과 국내 영향을 정리",
                    "core_message": f"글로벌 규제 동향: {reg_titles}",
                    "headline_draft": "글로벌 규제 변경 동향",
                    "domestic_angle": "해외 규제 변경의 국내 제도 영향",
                    "sources_used": ["KHIDI_GLOBAL_INFO"],
                    "publishable": "가능",
                    "priority": "medium",
                })
                _reg_added = True
                logger.info("[규제 고정] KHIDI 법령·허가 스토리 강제 추가")
        ...
        try:
            ...
            for src in sources_used:
                ...
                if resolved in filtered and resolved not in seen_sources:
                    original_data += format_for_prompt(resolved, filtered[resolved], include_counts=False) + "\n"
                    seen_sources.add(resolved)
                elif src in filtered and src not in seen_sources:
                    original_data += format_for_prompt(src, filtered[src], include_counts=False) + "\n"
