...
### 2. 같은 소스·같은 소재에서 기사 최대 1개
PMDA에서 기사 2개 이상 금지. 하나의 소스는 최대 1개 기사.
**같은 소재(예: PD-1 병용 특허)를 2개 기사로 분리하는 것도 금지.**
소재가 겹치면 하나로 통합하라. "특허 기사"와 "특허+평가 기사"를 따로 내면 안 된다.
나머지 기사는 반드시 다른 소스·다른 소재에서 뽑아야 한다.

### 3. "왜 중요한지"가 기사의 존재 이유
"어디서 뭘 승인했다"는 기사가 아니다. "그래서 국내에 어떤 영향이 있는가"가 기사다.

### 4. 핵심 논지 1문장 강제 (P0)
core_message는 **독자가 이 기사를 읽고 기억할 1문장**이다.
- 나쁜 예: "NICE TA 179건이 확인됐다" (사실 서술)
- 좋은 예: "같은 질환에서도 기업 제출 여부에 따라 권고가 갈리고 있다" (의미)
- 나쁜 예: "4월에 법안 24건이 발의됐다" (숫자 나열)
- 좋은 예: "4대 법률 동시개정으로 병원 처방·청구·관리가 한꺼번에 바뀔 수 있다" (영향)
core_message가 사실 서술이면 기사를 쓸 이유가 없다. 반드시 의미/영향/변화를 담아라.

### 5. 기사 유형 판정 (P4)
각 스토리에 article_type을 지정하라:
- "regulation": 규제/정책 기사 — "뭐가 바뀌는지" 중심, 영향받는 대상 명시
- "drug": 허가/급여 기사 — 약물 중심, 임상 데이터, 국내외 비교
- "industry": 특허/산업동향 기사 — 기술 축, 산업적 해석
...
- **보도자료 제목만 있고 본문이 없는 소재** — "식약처가 ~를 발표했다"만으로 기사를 쓸 수 없다. 발표 내용의 구체적 수치·대상·범위가 데이터에 포함되어 있을 때만 기사화.
- **법안 기사는 반드시 제안이유/주요내용 데이터가 있을 때만** — "~법 개정안이 발의됐다"만으로는 기사가 안 된다. 데이터에 [제안이유 및 주요내용] 컨텍스트가 포함돼 있으면 "뭐가 바뀌는지"를 중심으로 기사화.
...
[
  {
    ...
    "core_message": "독자가 기억할 핵심 1문장 — 반드시 의미/영향/변화를 담을 것",
    "headline_draft": "헤드라인 초안 (25자 이내, 의미 중심, 숫자 포함)",
