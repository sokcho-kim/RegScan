...
모든 데이터 소스(국내 규제, FDA, EMA 등)는 이 스키마로 정규화되어 메인화면에 표출됩니다.
...
type SourceType =
  ...
  | 'HIRA_GUIDELINE'       // 심사지침
  | 'MOHW_NOTICE'          // 복지부 고시
  | 'MOHW_ADMIN_NOTICE'    // 행정예고

  // 글로벌 규제
  | 'FDA_APPROVAL'         // FDA 승인
  | 'FDA_GUIDANCE'         // FDA 가이드라인
  | 'EMA_DECISION'         // EMA 결정
  | 'CMS_COVERAGE'         // CMS 급여 결정

  // 학술 (Phase 2)
  | 'PUBMED_ABSTRACT'      // PubMed 초록
...
│  📋 이번 주 주요 변경        │  🌐 글로벌 동향               │
│  ───────────────────        │  ───────────────────          │
│  [HIGH] 무릎관절 재료대...   │  [MID] FDA GLP-1 승인...      │
│  [MID] 물리치료 산정기준...  │  [LOW] EMA 안전성 권고...     │
│  [LOW] 검사료 고시 개정...   │                               │
│                             │                               │
├─────────────────────────────┴───────────────────────────────┤
│  📌 최근 본 규정  |  ⭐ 즐겨찾기  |  📊 나의 리포트          │
...
## 7. API 엔드포인트 (MedClaim 연동용)

```
GET /api/feed/cards
  - 쿼리: ?limit=10&domain=DRUG&impact_level=HIGH
  - 응답: FeedCard[]
...
GET /api/feed/today
  - 오늘의 변경사항 요약
  - 응답: { count: number, high_priority: FeedCard[], cards: FeedCard[] }
```
...
1. [ ] 데이터 소스별 수집 파이프라인 구현
2. [ ] 원문 → FeedCard 변환 로직
3. [ ] impact_level 자동 판정 규칙
4. [ ] why_it_matters 생성 (LLM 활용)
5. [ ] MedClaim API 연동 테스트
